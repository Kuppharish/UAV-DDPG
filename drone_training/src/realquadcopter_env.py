...
reg = register(
    ...
    entry_point='realquadcopter_env:QuadCopterEnv',
    timestep_limit=600,
    )
...
class QuadCopterEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.vel_pub = rospy.Publisher('/ardrone/cmd_vel', Twist, queue_size=5)
    ...
    def _reset(self):
        ...
        data_pose, data_vel, data_pr, data_bs_pos, data_bs_vel = self.take_observation()
        state, reward, done = self.process_data(data_pose, data_vel, data_pr, data_bs_pos, data_bs_vel)
    ...
    def _step(self, action):
        ...
        #z_vel=-0.15
        coef=1
        #if data_pose_1.pose.position.z<0.2:
        #    z_vel=1.5
        #    coef=0
        vel_cmd = Twist()
        '''
        ...
        data_pose, data_vel, data_pr, data_bs_vel, data_bs_pos = self.take_observation()
        #self.gazebo.pauseSim()

        # finally we get an evaluation based on what happened in the sim
        state,reward,done = self.process_data(data_pose, data_vel, data_pr, data_bs_vel, data_bs_pos)
    ...
    def take_observation(self):
        ...
        while data_pose is None:
            try:
                data_pose = rospy.wait_for_message('/vrpn_client_node/ardrone/pose', PoseStamped, timeout=5)
            except:
                continue
        ...
        while data_vel is None:
            try:
                data_vel = rospy.wait_for_message('/ardrone/odometry', Odometry, timeout=5)
            except:
                rospy.loginfo("Current drone vel not ready yet, retrying for getting robot vel")
        data_pr = 0
        '''
        while data_pr is None:
            try:
                data_pr_t = rospy.wait_for_message('/robot_bumper', Pressure, timeout=5)
                if len(data_pr_t.states)!=0:
                    data_pr=1
                else:
                    data_pr=0
            except:
                rospy.loginfo("Current base pressure not ready yet, retrying for getting robot pressure")
        '''
        data_bs_vel=None
        while data_bs_vel is None:
            try:
                data_bs_vel = rospy.wait_for_message('/odom', Odometry, timeout=5)
            except:
                rospy.loginfo("Current base imu not ready yet, retrying for getting base values")
        data_bs_pos= None
        while data_bs_pos is None:
            try:
                data_bs_pos = rospy.wait_for_message('/vrpn_client_node/base/pose', PoseStamped, timeout=5)
            except:
                continue
                #rospy.loginfo("Current drone imu not ready yet, retrying for getting robot imu")
        
        return data_pose, data_vel, data_pr, data_bs_vel, data_bs_pos
    ...
    def process_data(self, data_pos, data_vel, data_pr, data_bs_vel, data_bs_pos):
        base_pose=data_bs_pos.pose.position
        base_vel=data_bs_vel.twist.twist.linear
        drone_pose=data_pos.pose.position
        drone_vel=data_vel.twist.twist.linear
        p_x=drone_pose.x-base_pose.x
        p_y=drone_pose.y-base_pose.y
        p_z=drone_pose.z-base_pose.z
        data_vel=data_vel.twist.twist
        v_x=drone_vel.x-base_vel.x
        v_y=drone_vel.y-base_vel.y
        a_x=self.act[0]
        a_y=self.act[1]
        c=0
        if abs(p_x)<0.5 and abs(p_y)<0.5 and p_z<0.3:
            c=1
        self.pr=self.pr+c
        ...
        altitude_bad_1 = drone_pose.z > self.max_altitude
        altitude_bad_2 = drone_pose.z < self.min_altitude
        x_bad=abs(drone_pose.x)>self.max_x
        x_bad=False
        y_bad=abs(drone_pose.y)>self.max_y
